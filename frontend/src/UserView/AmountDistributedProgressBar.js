...
function AmountDistributedProgressBar(props) {
    ...
    return (
        <div style={{
            ...
            <Box>
                <Typography variant="body1" color="textPrimary">Amount distributed:</Typography>
                <Box display="flex" alignItems="center">

                    <Box width="65%" mr={2}>
                        <LinearProgress variant="determinate" value={progress} />
                    </Box>
                    <Typography variant="body1" color="textSecondary">{`${amountDistributed
                        }/${amountTotal} $`}</Typography>
                </Box>
            </Box>
        </div>
    );
